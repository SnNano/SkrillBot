     ...
     <section className="flex justify-center flex-col lg:pb-32 lg:pt-6 md:pb-12 md:pt-4 sm:py-6">
      <div className="grid grid-cols-1 gap-3"> 
        <form onSubmit={handleSubmit}>
            <div className="mb-6">
                <label htmlFor="language" className="block mb-2 text-sm font-medium text-green-500">Language (Javascript, java, cpp)</label>
                <input maxLength={100} minLength={1} name="language" id="language" value={language} onChange={handleChange} className="block w-full px-5 py-3 text-base text-gray-900 placeholder-gray-500 border border-gray-300 rounded-md shadow-sm focus:outline-0 flex-1" placeholder="Language (Javascript, java, cpp)"/>
            </div>      
            <div className="mb-6">
                <label htmlFor="codeEx" className="block mb-2 text-sm font-medium text-green-500">Describe exactly the exercice you wanna solve for better results</label>
                <textarea minLength={3} maxLength={500} rows="6" name="codeEx" id="codeEx" value={codeEx} onChange={handleChange} className="block w-full px-5 py-3 text-base text-gray-900 placeholder-gray-500 border border-gray-300 rounded-md shadow-sm focus:outline-0 flex-1" placeholder="Write a Python function that takes in a list of numbers and returns the sum of the even numbers in the list."></textarea>
            </div>
