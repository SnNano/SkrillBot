...
const QandA = () => {
  ...
  return (
    <>
      ...
      <Sidebar />
      <h3 className="lg:text-4xl sm:text-2xl font-bold text-center mt-6 leading-relaxed">Start Generating Now <span className="text-indigo-400">Replies For anything</span></h3>
              ...
              return <div key={index} className={`flex w-full mt-2 space-x-3 max-w-xs ${data.user === "Q" ? 'ml-auto justify-end' : ''}`}>
                ...
                <div>
                  <div className={`${data.user === "Q" ? 'bg-indigo-400 text-white' : 'bg-gray-300'} p-3 rounded-r-lg rounded-bl-lg`}>
                    ...
                    <span className="cursor-pointer text-indigo-400 text-sm" onClick={() => { copyToClip(data.message) }}><i className="fa-regular fa-copy"></i> Copy</span>
