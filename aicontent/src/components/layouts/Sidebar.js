      ...
      <aside className={`fixed top-0 left-0 z-40 w-64 ${open ? '' : '-translate-x-full'} h-screen pt-4 transition-transform bg-white shadow-md sm:translate-x-0`}>
        <div className="overflow-hidden h-full px-3 pb-4 bg-white relative">
          <Link to="/" className="flex items-center ml-4">
            <span className="text-2xl text-center font-semibold whitespace-nowrap">SkrillBot</span>
          </Link>
          <ul className="space-y-2 mt-6">
            {sidebarData.map((item) => {
              return <li key={item.id}>
                <Link to={item.link} className="flex my-6 items-center p-2 text-sm font-normal text-gray-900 rounded-lg hover:bg-indigo-500 hover:text-white">
